...
//intake functions
  ...
  void sort2(bool red, int timeOut = 5000){//spits the ball from the back
    //trans.spinFor(forward, 0.5, turns);
    trans.spin(forward);
    shot.spin(forward);
    cl.spin(forward);
...
//autons
  ...
  void homeRow(){
    ...
    cl.stop();
    waitUntil(checkColor(red, false));//wait until a ball passes through the sensor
    cl.stop();
    ...
    //3
    wait(0.5, seconds);//if out of time, delete that
    drive(-700, 80);//700
    iTurnPID(270, 1200);
    ...
    trans.spin(forward);
    shot.spin(forward);
    waitUntil(checkColor(red, true));//same as above
    cl.spinFor(reverse, 5, turns, false);
  ...
  void singleUniformL(void){
    ...
    sort1(!red);
    cl.spinFor(reverse, 7, turns, false);//outtake from front
    ...
    drive(350, false, 60);  
    sort2(!red, 5000);
    //shot.spinFor(reverse, 3, turns);//from back
  ...
  void skills(){
    ...
    //side 1
      ...
      shot.spinFor(reverse, 8, turns, false);//back out
    ...
    //side 2
      cl.spinFor(reverse, 7, turns, false);//continue front out
      trans.spinFor(reverse, 7, turns, false);
    ...
    //corner 3
      cl.spinFor(reverse, 7, turns, false);//front out
      trans.spinFor(reverse, 7, turns, false);
...
void control(void){
  ...
  while (true) {
    ...
    //intake
    if(Controller1.ButtonL2.pressing()){cl.spin(forward);}
    else if(Controller1.ButtonL1.pressing()){cl.spin(reverse);}
    else{cl.stop();}

    if(Controller1.ButtonR2.pressing()){//shoot
      trans.spin(forward);
      shot.spin(forward);
    }
    else if(Controller1.ButtonR1.pressing()){//back spit, while top part still shoots
      trans.spin(forward);
      shot.spin(reverse);
    }else if(Controller1.ButtonB.pressing()){//reverse the whole system
      shot.spin(reverse);
      trans.spin(reverse);
    }else{
      trans.stop();
